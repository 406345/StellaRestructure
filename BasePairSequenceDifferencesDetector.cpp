...
inline int BasePairSequenceDifferencesDetector::NeedlemanWunsch(string & f1, string & f2, string & sequenceA, string & sequenceB, string & sequenceM, int gap_open, int gap_extn)
{
	...
	*/
	...
	val(0, 0) = 0;
	val(1, 0) = gap_open; // open end penalty of read 1
	...
	val(0, 1) = 0; // open end penalty of read 2
	for (j = 2; j<f2.length(); j++) {
		val(0, j) = val(0, j - 1) + 0;
	}
	for (j = 1; j<f2.length(); j++) {
	...
	double identity = L_id*1.0/(f2.size()-1);
	int fina_score = 0; //val[f1.size()-1][f2.size()-1];

	cout<<string("Alignment score=") << fina_score <<endl;
	cout<<string("Length of sequence 1:") << (f1.size()-1)<<endl;
	cout<<"Length of sequence 2:" << (f2.size()-1)<<endl;
	cout<<"Aligned length      :" << L_ali <<endl;
	cout<<"Identical length    :" << L_id<<endl;

	cout<<"Sequence identity=" << identity<<endl;
	cout<<" " << L_id  << "/" << (f2.length()-1)<<endl;
	*/
	...
	while (true) {
		...
		if ((i>(f1.length() - 1)) && (j<(f2.length() - 1)))     // unaligned C on 1
		{
			...
			if (k<buf_size) {
				...
				//stella::logger::print(cerr, "fatal error, out-of-bound in dynamic progreamming");
			}
			j = j + 1;
		}
	...
	/*
	string CIGAR;
	for(int i=1; i<sequenceM.size(); i++){
	if(sequenceM[i]==0) break;
	char a = sequenceM[i];
	int c=0;
	for(;i<sequenceM.size();i++){
	if(sequenceM[i]==a) c++;
	else {i--; break;}
	}
	CIGAR += a + std::to_string(c);
	}
